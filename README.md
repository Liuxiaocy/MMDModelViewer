...
- 🧹 **缓存目录安全命名**：缓存模型/动作的目录与文件名只保留 ASCII 字符（字母/数字/`._-`），
  中文、空格与 `% # ?` 等特殊字符一律替换为下划线，避免 `mmd://` URL 解析与文件系统异常
- 🌆 **场景卡片**：识别自 `D:\素材\3D模型\场景` 的缓存模型会显示在左侧「场景」卡片中（已缓存场景组）
- 🎬 **场景 + 角色同时预览**：场景模型与角色模型可共存于 3D 视口
  - 从左侧模型/场景/缓存列表直接**拖拽**到视口，模型会落在鼠标指向的地面位置
  - 点击工具栏「🖐 移动模式」后，可在视口内**按住左键拖动**任意模型到场景任意位置
  - 「🧹 清空场景」一键移除已加入的场景模型（保留当前角色模型）
